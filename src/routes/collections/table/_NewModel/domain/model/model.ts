...
export class NewModel {
    ...
    static fromJSON(json: any): NewModel {
        ...
        const object = new NewModel();

        object.DocId = json['DocId'];

        object.I000 = json['I000'] ?? 0;
        ...
        //
        // object.S000 = json['S000'] ?? '';
        // object.S001 = json['S001'] ?? '';
    ...
    static fromJSONString(jsonString : string) : NewModel {
        if(typeof jsonString === 'string'){
            return NewModel.fromJSON(JSON.parse(jsonString));
        }else{
            return NewModel.fromJSON(jsonString);
        }

    }


    //_NewModel 배열 자체를 직렬화/역직렬화 해야할때, -> get_all 등을 할때 사용
    static toJSONArrayString(array : NewModel[]) : string {
        return JSON.stringify(array.map(obj => obj.toJSON()));
    }

    static fromJSONArrayString(jsonString : any) : NewModel[] {
        if(typeof jsonString === 'string'){
            return JSON.parse(jsonString).map(NewModel.fromJSON);
        }else{
            return jsonString.map(NewModel.fromJSON);
        }
